...
| Module | Change |
|--------|--------|
| `ingest.rs` | Detect non-.md files in bundle folders as assets |
| `markdown.rs` | `promote_to_bundle(slug)` — moves `{slug}.md` to `{slug}/index.md` |
| `search.rs` | Update slug resolution to check `index.md` variant |
| `mcp.rs` | Expose bundle assets as MCP resources; `wiki_write` handles asset content |
...
| Asset writing via `wiki_write` | **not implemented** |
